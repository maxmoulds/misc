...
__USER="mtmoulds"
#Today's 3:
# 2) change  the scale (cirp something) to 1/mirp
# 3) start collect logic for graph
# 4) start run folder logic
# 5) coefs concat
# 6) sigplot and signu calls

function __version() {
cat << EOF
####### WELCOME TO THE ULTIMATE-ISH CHYMERA RUN SCRIPT(S)
# WORKS WITH point (chymera-uo)
# WILL* WORKS WITH resolved (chymera-whit)
# FUTURE support for linear will be somewhat easy. 
...
# for this script
#
# POLYOUT SUPPORT IS ON THE WAY
#
#
# VERSION 0.01 - 15-9-13 - MM
EOF
}
function __help() {
cat << EOF
...
#echo ""
#logvv " ------ TEST generating hashes --- should not be used unless your sure"
#md5_sum_gen $RUN_DIR
#echo ""
#md5_sum_check $RUN_DIR
#echo -e "\n\n\n"
# SPACE OUT MAN
#point_load_var $RUN_DIR 
#echo ""
#logvv "Man, it was a good run!"

#graph tests#
input_parse "$@"
__MODEL_NAME="resolved"
__RUN_NAME="test"
resolved_plotmfrac ~/physics/nonlinear/resolved/runM1r ./graphs
resolved_plotit ~/physics/nonlinear/resolved/runM1r ./graphs
resolved_equilib_contour ~/physics/nonlinear/resolved/runM1r/equilib ./graphs
#resolved_signu ./sigin ./sigout # need expains

__MODEL_NAME="point"
__RUN_NAME="test"
#point_hscf_contour ./point/runM1p/hscf ./graphs


### END debug tempspace garbage fun words
...
__POINT_VAR_FILES=(headers/hydroparam.h radhydro/io.f radhydro/3dhyd-main.f $__RUN_NAME/fort.5)
__RESOLVED_LIBS=(bin)

## RUN VARIABLES ##
POINT_KMAX=-1		# from headers/hydroparam.h
POINT_JMAX=-1		# from headers/hydroparam.h
POINT_LMAX=-1		# from headers/hydroparam.h
POINT_IOSTEP=-1		#
POINT_KTOP=-1		#
POINT_JTOP=-1		#
POINT_RHOMAX=-1		#
POINT_RMINMAX=-1	# 
POINT_POT3KMAX=-1	# from headers/hydroparam.h
POINT_POT3JMAX=-1	# from headers/hydroparam.h
POINT_HJ=-1		# from headers/hydroparam.h
POINT_HK=-1		# from headers/hydroparam.h
POINT_LRLMAX=-1		# from headers/hydroparam.h
POINT_LRJMAX=-1		# from headers/hydroparam.h
POINT_LRKMAX=-1		# from headers/hydroparam.h
POINT_ITABLE=-1		# from headers/hydroparam.h
POINT_JMIN=-1		# from headers/hydroparam.h
POINT_IOF_STAR_MASS=-1	# hardwired star_mass in radhydro/io.f 
...
#__POINT_VAR_FILES="headers/hydroparam.h radhydro/io.f radhydro/3dhyd-main.f $__RUN_NAME/fort.5"
#__RESOLVED_LIBS="bin"
function point_load_var() {
# $1 is the point model "ROOT" $2 is the special __RUN_NAME optional
# First the header file!!
local varlist
logvv "grepping $1/${__POINT_VAR_FILES[0]}"
# $ egrep -o '\bparameter \(kmax=[0-9][0-9][0-9][0-9]?\b' ./hydroparam.h | egrep -o '\b[0-9][0-9][0-9][0-9]?\b'
# better, deals with comments
# $ egrep -o '^[^#;!c]*\bparameter \(jmax=[0-9][0-9][0-9][0-9]?\b' ./hydroparam.h | egrep -o '\b[0-9][0-9][0-9][0-9]?\b'
if (( ("$POINT_KMAX" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_KMAX=$(egrep -o '^[^#;!c]*\bparameter \(kmax=[0-9][0-9][0-9][0-9]?\b' "$1/${__POINT_VAR_FILES[0]}" | egrep -o '\b[0-9][0-9][0-9][0-9]?\b')
logvv "grep found (kmax) = $POINT_KMAX in $1/${__POINT_VAR_FILES[0]}" 
else
logvv "KMAX was already set"
fi
if (( ("$POINT_JMAX" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_JMAX=$(egrep -o '^[^#;!c]*\bparameter \(jmax=[0-9][0-9][0-9][0-9]?\b' "$1/${__POINT_VAR_FILES[0]}" | egrep -o '\b[0-9][0-9][0-9][0-9]?\b')
logvv "grep found (jmax) = $POINT_JMAX in $1/${__POINT_VAR_FILES[0]}"
else
logvv "JMAX was already set"
fi
if (( ("$POINT_LMAX" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_LMAX=$(egrep -o '^[^#;!c]*\bparameter \(lmax=[0-9][0-9][0-9][0-9]?\b' "$1/${__POINT_VAR_FILES[0]}" | egrep -o '\b[0-9][0-9][0-9][0-9]?\b')
logvv "grep found (lmax) = $POINT_LMAX in $1/${__POINT_VAR_FILES[0]}"
else
logvv "LMAX was already set"
fi
# Since pot3 vars are coded differently
# $ egrep -o '^[^#;!c]*\bparameter\(pot3kmax=[0-9][0-9][0-9][0-9]?\b,'
if (( ("$POINT_POT3KMAX" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_POT3KMAX=$(egrep -o '^[^#;!c]*\bparameter\(pot3kmax=[0-9][0-9][0-9][0-9]?\b,' "$1/${__POINT_VAR_FILES[0]}" | egrep -o '\b[0-9][0-9][0-9][0-9]?\b')
logvv "grep found (pot3kmax) = $POINT_POT3KMAX in $1/${__POINT_VAR_FILES[0]}"
else
logvv "POT3KMAX was already set"
fi
if (( ("$POINT_POT3JMAX" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_POT3JMAX=$(egrep -o '^[^#;!c]*\bparameter\(pot3jmax=[0-9][0-9][0-9][0-9]?\b,' "$1/${__POINT_VAR_FILES[0]}" | egrep -o '\b[0-9][0-9][0-9][0-9]?\b')
logvv "grep found (pot3jmax) = $POINT_POT3JMAX in $1/${__POINT_VAR_FILES[0]}"
else
logvv "POT3JMAX was already set"
fi
#now some hj,hk,
# cuz im cool
# egrep -o '^[^#;!c]*\bparameter \(hj=[0-9][0-9][0-9][0-9]?,hk=[0-9][0-9][0-9][0-9]?\b,' ./hydroparam.h | egrep -o '\b[0-9][0-9][0-9][0-9]?\b' | head -1
if (( ("$POINT_HJ" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_HJ=$(egrep -o '^[^#;!c]*\bparameter \(hj=[0-9][0-9][0-9][0-9]?,hk=[0-9][0-9][0-9][0-9]?\b,' "$1/${__POINT_VAR_FILES[0]}" | egrep -o '\b[0-9][0-9][0-9][0-9]?\b' | head -1)
logvv "grep found (hj) = $POINT_HJ in $1/${__POINT_VAR_FILES[0]}"
else
logvv "hj was already set"
fi
# egrep -o '^[^#;!c]*\bparameter \(hj=[0-9][0-9][0-9][0-9]?,hk=[0-9][0-9][0-9][0-9]?\b,' ./hydroparam.h | egrep -o '\b[0-9][0-9][0-9][0-9]?\b' | tail -1
if (( ("$POINT_HK" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_HK=$(egrep -o '^[^#;!c]*\bparameter \(hj=[0-9][0-9][0-9][0-9]?,hk=[0-9][0-9][0-9][0-9]?\b,' "$1/${__POINT_VAR_FILES[0]}" | egrep -o '\b[0-9][0-9][0-9][0-9]?\b' | tail -1)
logvv "grep found (hk) = $POINT_HK in $1/${__POINT_VAR_FILES[0]}"
else
logvv "hk was already set"
fi
### HAHA please don't laugh, or do. 
#egrep -o '^[^#;!c]*\bparameter \(lrlmax=[0-9][0-9][0-9]?[0-9]?,lrjmax=[0-9][0-9][0-9]?[0-9]?,lrkmax=[0-9][0-9]?[0-9]?[0-9]?\b' ./hydroparam.h | egrep -o '\b[0-9][0-9]?[0-9]?[0-9]?\b' | head -2 | tail -1
if (( ("$POINT_LRLMAX" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_LRLMAX=$(egrep -o '^[^#;!c]*\bparameter \(lrlmax=[0-9][0-9][0-9]?[0-9]?,lrjmax=[0-9][0-9][0-9]?[0-9]?,lrkmax=[0-9][0-9]?[0-9]?[0-9]?\b' "$1/${__POINT_VAR_FILES[0]}" | egrep -o '\b[0-9][0-9]?[0-9]?[0-9]?\b' | head -1)
logvv "grep found (lrlmax) = $POINT_LRLMAX in $1/${__POINT_VAR_FILES[0]}"
else
logvv "lrlmax was already set"
fi
if (( ("$POINT_LRJMAX" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_LRJMAX=$(egrep -o '^[^#;!c]*\bparameter \(lrlmax=[0-9][0-9][0-9]?[0-9]?,lrjmax=[0-9][0-9][0-9]?[0-9]?,lrkmax=[0-9][0-9]?[0-9]?[0-9]?\b' "$1/${__POINT_VAR_FILES[0]}" | egrep -o '\b[0-9][0-9]?[0-9]?[0-9]?\b' | head -2 | tail -1)
logvv "grep found (lrjmax) = $POINT_LRJMAX in $1/${__POINT_VAR_FILES[0]}"
else
logvv "lrjmax was already set"
fi
if (( ("$POINT_LRKMAX" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_LRKMAX=$(egrep -o '^[^#;!c]*\bparameter \(lrlmax=[0-9][0-9][0-9]?[0-9]?,lrjmax=[0-9][0-9][0-9]?[0-9]?,lrkmax=[0-9][0-9]?[0-9]?[0-9]?\b' "$1/${__POINT_VAR_FILES[0]}" | egrep -o '\b[0-9][0-9]?[0-9]?[0-9]?\b' | tail -1)
logvv "grep found (lrkmax) = $POINT_LRKMAX in $1/${__POINT_VAR_FILES[0]}"
else
logvv "lrkmax was already set"
fi
# egrep -o '^[^#;!c]*\bparameter \(jmin=[0-9][0-9]?[0-9]?[0-9]?\b,' ./hydroparam.h | egrep -o '\b[0-9][0-9]?[0-9]?[0-9]?\b'
if (( ("$POINT_JMIN" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_JMIN=$(egrep -o '^[^#;!c]*\bparameter \(jmin=[0-9][0-9]?[0-9]?[0-9]?\b,' "$1/${__POINT_VAR_FILES[0]}" | egrep -o '\b[0-9][0-9]?[0-9]?[0-9]?\b')
logvv "grep found (jmin) = $POINT_JMIN in $1/${__POINT_VAR_FILES[0]}"
else
logvv "jmin was already set"
fi
#itable?
# egrep -o '^[^#;!c]*\bparameter \(itable=[0-9][0-9]?[0-9]?[0-9]?\b' ./hydroparam.h | egrep -o '\b[0-9][0-9]?[0-9]?[0-9]?\b'
if (( ("$POINT_ITABLE" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_ITABLE=$(egrep -o '^[^#;!c]*\bparameter \(itable=[0-9][0-9]?[0-9]?[0-9]?\b' "$1/${__POINT_VAR_FILES[0]}" | egrep -o '\b[0-9][0-9]?[0-9]?[0-9]?\b')
logvv "grep found (itable) = $POINT_ITABLE in $1/${__POINT_VAR_FILES[0]}"
else
logvv "itable was already set"
fi

# MOVING ON TO io.f real quick
# egrep -o '^[^#;!c]*\bmass_star = [0-9]?[0-9]?[0-9]\.[0-9]?[0-9]?[0-9]?\b' ./io.f | egrep -o '\b[0-9]?[0-9]?[0-9]\.[0-9][0-9]?[0-9]?\b'
if (( ("$POINT_IOF_STAR_MASS" == "-1") && ("$__POINT_OVERRIDE" == "0") )); then
POINT_IOF_STAR_MASS=$(egrep -o '^[^#;!c]*\bmass_star = [0-9]?[0-9]?[0-9]\.[0-9]?[0-9]?[0-9]?\b' "$1/${__POINT_VAR_FILES[1]}" | egrep -o '\b[0-9]?[0-9]?[0-9]\.[0-9][0-9]?[0-9]?\b')
logvv "grep found (star mass in io.f) = $POINT_IOF_STAR_MASS in $1/${__POINT_VAR_FILES[0]}"
else
logvv "star mass from io.f was already set"
fi
...
set ylabel "Label2"
set term png
set output "${FILE}-SOMENAME-${__MODEL_NAME}-${__RUN_NAME}-${__TIME}.png"
plot "${FILE}" using 1:2:3:4 with errorbars title "SOMENAME-${__MODEL_NAME}-${__RUN_NAME}-${FILE}"
EOF
done
unset __tempin
unset __tempout
...
EOF
logv "rhomax was $(point_rhomax $__tempin)"
unset __tempin
unset __tempout
}
...
scale = $(resolved_cirp $__tempin/equilib)
set logscale y
set key t l
set multiplot
set size 1.0,0.5
...
unset multiplot
EOF
logv "cirp (scale) was $(resolved_cirp $__tempin/equilib)"
unset __tempout
unset __tempin
}
...
### EWWWW HELPERS (see web-info.sh)

function point_rhomax() {
  logv "$1"
  if [ -a $1/polyout ]; then
...
function resolved_cirp() {
  logv "$1"
  shopt -s nullglob
  if [ -a $1/polyout ]; then
    val=$(grep -w CIRP: "$1/polyout" | egrep -o '[0-9]?.?[0-9]{1,9}E\+[0-9]{2}')
    logv "$val was val"
    exp=$val
    base=$val
    exp=${exp: -2}
    base=${base%%E}
    cirp=`echo "scale=4; ($base/1)*10^(0$exp)" | bc`
    echo $cirp
  else
    logv "polyout not found"
    exit $__ERR
...
function resolved_mirp() {
  logv "$1"
  shopt -s nullglob
  if [ -a $1/polyout ]; then
    val=$(grep -w MIRP: "$1/polyout" | egrep -o '[0-9]?.?[0-9]{1,9}E\+[0-9]{2}')
    logv "$val was val"
    exp=$val
    base=$val
    exp=${exp: -2}
    base=${base%%E}
    mirp=`echo "scale=4; ($base/1)*10^(0$exp)" | bc`
    echo $mirp
  else
    logv "polyout not found"
    exit $__ERR
