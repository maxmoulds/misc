...
#	-pass 1 -map_metadata 0 -map_chapters 0 -c:v \
...
#	-an -pix_fmt yuv422p10le -c:a copy -c:s copy -f rawvideo \
#	-y /dev/null \
...
#( time ( export FFREPORT=file="$FFMPEG_LOG_LOC" && \
#        ffmpeg -report -i "$INPUT_DIR/$FILE" -ss $STARTING_FRAME_SECONDS -to $(($STARTING_FRAME_SECONDS + $DURATION)) \
#        -pass 2 -map_metadata 0 -map_chapters 0 -c:v \
#        libx265 -preset medium \
...
#        -pix_fmt yuv422p10le -c:a copy -c:s copy -f matroska \
...
#start command
( time ( export FFREPORT=file="$FFMPEG_LOG_LOC" \
	&& ffmpeg -report -i "$INPUT_DIR/$FILE" \
	-map_metadata 0 -map_chapters 0 -c:v libx265 -preset medium \
	-x265-params preset=medium:profile=main10:tune=grain,film \
       	-sws_flags lanczos -pix_fmt yuv422p10le -c:a copy -c:s copy \
	-f matroska "$VIDRUN_DIR/$filename$GRP_NAME.$OUTPUT_DIR_EXT" ) &> "$SCRIPT_LOG_LOC" ) &> "$TIME_LOG_LOC"
#end command
